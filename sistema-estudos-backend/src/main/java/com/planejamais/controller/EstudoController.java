...
public class EstudoController {
    ...
    @PostMapping("/heatmap/sync")
    public ResponseEntity<PomodoroResponse> sincronizarHeatmap(
            @AuthenticationPrincipal UserDetails user,
            @RequestBody HeatmapSyncRequest request
    ) {
        return ResponseEntity.ok(estudoService.sincronizarHeatmap(user.getUsername(), request));
    }
}
